...
- (NSString*)getUuid {
    ...
    if ([deviceId isKindOfClass:[NSData class]]) {
        deviceString = [[[NSString alloc] initWithData:deviceId encoding:NSUTF8StringEncoding] autorelease];
    }
    else {
...
- (SCMatch*)joinPrivateMatch:(NSString*)password withOptions:(NSString*)options {
    ...
    if ([self didRequestSucceed:serverMatch]) {
        result = [[[SCMatch alloc] initWithPlayerId:uuid andMatchmaker:self andMatchId:[serverMatch objectForKey:@"id"] andPassword:password] autorelease];
...
- (SCMatch*)autoJoinMatchWithMaxPlayers:(int)numPlayers creatingIfNecessary:(BOOL)createIfNecessary withOptions:(NSString*)gameOptions {
    ...
    if ([self didRequestSucceed:autoMatch]) {
        ...
        result = [[[SCMatch alloc] initWithPlayerId:uuid andMatchmaker:self andMatchId:[autoMatch objectForKey:@"id"]] autorelease];
        ...
        result = [[[SCMatch alloc] initWithPlayerId:uuid andMatchmaker:self] autorelease];
        int port = [result becomeServer];
        if (port > 0) {
...
- (NSDictionary*)callMethod:(ApiMethod)method withParams:(NSDictionary*)params {
    ...
    NSString* jsonText = [[[[NSString alloc] initWithData:responseText encoding:NSASCIIStringEncoding] autorelease] trim];
            ...
            @autoreleasepool {
                ...
                if ([self didRequestSucceed:waitingProxies] && [[waitingProxies objectForKey:@"numWaiting"] intValue] > 0) {
                    ...
                    for (int connection = 0; connection < numWaiting; connection++) {
                        ...
                        [bridgeArray addObject:[[[SocketBridge alloc] initWithServerInput:input andServerOutput:output andClientInput:localInput andClientOutput:localOutput] autorelease]];
                    }
                }
