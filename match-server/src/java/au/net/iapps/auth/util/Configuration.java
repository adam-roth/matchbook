...
public class Configuration {
	...
	static {
		String host = System.getProperty("au.net.iapps.server.hostname", "localhost").toLowerCase();
		String port = System.getProperty("au.net.iapps.server.port", "8080");
		String protocol = System.getProperty("au.net.iapps.server.protocol", "http").toLowerCase();
		
		String localEmail = System.getProperty("au.net.iapps.server.email.local", "false");
		LOCALHOST_CAN_SEND_EMAIL = "true".equals(localEmail);
		ADMIN_EMAIL_ACCOUNT = System.getProperty("au.net.iapps.server.email.admin", "admin@iapps.net.au").toLowerCase();
		ADMIN_EMAIL_NAME = System.getProperty("au.net.iapps.server.email.admin.name", "iApps Admin");
		
		String numLocks = System.getProperty("au.net.iapps.server.num.locks", "4096");
		NUM_ACCOUNT_LOCKS = Integer.parseInt(numLocks);
		
		PERSISTENCE_UNIT = System.getProperty("au.net.iapps.db.name", "iapps_devel");
