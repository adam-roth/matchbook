...
function $pwdMeter() {
	...
	for (var i=0; i < arguments.length; i++) {
		var elm = arguments[i];
		if (typeof(elm == "string")) { elm = document.getElementById(elm) ? document.getElementById(elm) : {parentNode: {}, style:{}}; }
		if (arguments.length == 1) { return elm ? elm : {parentNode: {}, style:{}}; }
		arrElms.push(elm);
	}
	return arrElms ? arrElms : {parentNode: {}, style:{}};
}
...
function chkPass(pwd) {
	var oScorebar = $pwdMeter("scorebar");
	var oScore = $pwdMeter("score");
	var oComplexity = $pwdMeter("complexity");
	...
	/* Modify overall score value based on usage vs requirements */
		...
		$pwdMeter("nLengthBonus").innerHTML = "+ " + nScore; 
		...
		$pwdMeter("nAlphaUCBonus").innerHTML = sAlphaUC; 
		$pwdMeter("nAlphaLCBonus").innerHTML = sAlphaLC;
		$pwdMeter("nNumberBonus").innerHTML = sNumber;
		$pwdMeter("nSymbolBonus").innerHTML = sSymbol;
		$pwdMeter("nMidCharBonus").innerHTML = sMidChar;
		...
		$pwdMeter("nAlphasOnlyBonus").innerHTML = sAlphasOnly; 
		$pwdMeter("nNumbersOnlyBonus").innerHTML = sNumbersOnly; 
		$pwdMeter("nRepCharBonus").innerHTML = sRepChar; 
		$pwdMeter("nConsecAlphaUCBonus").innerHTML = sConsecAlphaUC; 
		$pwdMeter("nConsecAlphaLCBonus").innerHTML = sConsecAlphaLC; 
		$pwdMeter("nConsecNumberBonus").innerHTML = sConsecNumber;
		$pwdMeter("nSeqAlphaBonus").innerHTML = sSeqAlpha; 
		$pwdMeter("nSeqNumberBonus").innerHTML = sSeqNumber; 
		$pwdMeter("nSeqSymbolBonus").innerHTML = sSeqSymbol; 
		...
		for (var c=0; c < arrCharsLen; c++) {
			var oImg = $pwdMeter('div_' + arrCharsIds[c]);
			var oBonus = $pwdMeter(arrCharsIds[c] + 'Bonus');
			$pwdMeter(arrCharsIds[c]).innerHTML = arrChars[c];
		...
		$pwdMeter("nRequirementsBonus").innerHTML = sRequirements;
	...
	window._passwordCheckScore = nScore;
}

function togPwdMask() {
	var oPwd = $pwdMeter("passwordPwd");
	var oTxt = $pwdMeter("passwordTxt");
	var oMask = $pwdMeter("mask");
	if (oMask.checked) { 
...
function initPwdChk(restart) {
	...
	for (var i in arrZeros) { $pwdMeter(arrZeros[i]).innerHTML = "0"; }
	for (var i in arrPassPars) { $pwdMeter(arrPassPars[i]).parentNode.className = "pass"; }
	for (var i in arrPassDivs) { $pwdMeter(arrPassDivs[i]).className = "pass"; }
	for (var i in arrFailPars) { $pwdMeter(arrFailPars[i]).parentNode.className = "fail"; }
	for (var i in arrFailDivs) { $pwdMeter(arrFailDivs[i]).className = "fail"; }
	$pwdMeter("passwordPwd").value = "";
	$pwdMeter("passwordTxt").value = "";
	$pwdMeter("scorebar").style.backgroundPosition = "0";
	if (restart) {
		$pwdMeter("passwordPwd").className = "";
		$pwdMeter("passwordTxt").className = "hide";
		$pwdMeter("mask").checked = true;
	}
}
